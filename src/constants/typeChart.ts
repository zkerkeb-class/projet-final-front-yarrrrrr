...
export function getTypeEffectiveness(attackType: string, defenderTypes: string[]): number {
  ...
  for (const defType of defenderTypes) {
    const typeData = TYPE_CHART.find(t => t.name === defType);
    if (!typeData) continue;
    if (typeData.immunes.includes(attackType)) {
      return 0; // immune to any type means 0 damage
    }
    if (typeData.weaknesses.includes(attackType)) {
      multiplier *= 0.5;
    } else if (typeData.strengths.includes(attackType)) {
      multiplier *= 2;
    }
  }
  return multiplier;
